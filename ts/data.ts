 ...
 */
const setPageHTML = (values: any): string => {
  let item = ''
  const page = values[0]
  const data = values[1]
    ...
    case 'projects':
      ...
      if (imgPieces) {
        ...
        imgPieces.forEach((img: string) => {
          ...
          pieces.forEach((innerItem: string) => {
 ...
 * @param {Record<string, unknown>=} next - (optional) - link to next page of results
 * @param {Record<string, unknown>=} prev - (optional) - link to previous page of the results
 */
const renderPage = (
  data: any,
  page: string,
  ...
  if (page === 'contact') {
    ...
    if (location.toString().indexOf('submitted') === -1) {
      await fetch(`${utilityJS.API_BASE}/contact/feedback`) // get the feedback form as text
        ...
        .then((innerPage) => {
          data = innerPage.replace(/\/drupal8/g, utilityJS.API_BASE) // update the HTML URLs from relative to absolute
      ...
      switch (page) {
        ...
        case 'courses':
          if (search) {
            ...
            data = await getData(
              `${utilityJS.API_BASE}/jsonapi/node/awards?sort=-field_award_date,title&
                include=field_award_pdf,field_track_image,field_award_images&
                page[limit]=${utilityJS.MAX_ITEMS_PER_PAGE}`
            )
          }
